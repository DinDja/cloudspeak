...
export default function PropertiesPanel({ slide }) {
  ...
  const [preset, setPreset] = useState('Grafite')
  ...
  return (
    <div className="space-y-5">
      ...
      <Section label="Status da sala">
        <div className="grid grid-cols-2 gap-2"><Stat color="slate" label="Pública" value="Ativa" icon={Eye} /><Stat color="green" label="Respostas" value="Abertas" icon={Lock} /></div>
      </Section>
    </div>
  )
}
  ...
  return (
    <>
      <Section label="Resumo da etapa">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-stone-100 text-stone-700"><Hash className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-800">{slide.question || 'Sem pergunta'}</p><p className="mt-0.5 text-xs text-slate-500">{type?.label ?? 'Etapa'}</p></div></div></div>
      </Section>
...
function DesignTab({ preset, setPreset }) {
  return (
    <>
      <Section label="Cor de destaque"><div className="grid grid-cols-3 gap-2 rounded-lg border border-slate-200 bg-white p-3 shadow-sm">{COLOR_PRESETS.map((item) => <button key={item.name} type="button" onClick={() => setPreset(item.name)} className={['flex flex-col items-center gap-2 rounded-lg p-2 transition-colors', preset === item.name ? 'bg-stone-100 ring-2 ring-stone-400' : 'hover:bg-slate-50'].join(' ')}><span className={['h-7 w-7 rounded-full', item.color].join(' ')} /><span className="text-[11px] font-medium text-slate-600">{item.name}</span></button>)}</div></Section>
      <Section label="Fundo"><div className="grid grid-cols-2 gap-2 rounded-lg border border-slate-200 bg-white p-3 shadow-sm">{['Claro', 'Noite', 'Areia', 'Azul'].map((background) => <button key={background} type="button" className="overflow-hidden rounded-lg border border-slate-200 hover:border-stone-400"><div className={['h-12', background === 'Claro' ? 'bg-white' : background === 'Noite' ? 'bg-slate-800' : background === 'Areia' ? 'bg-amber-100' : 'bg-blue-100'].join(' ')} /><p className="bg-white py-1.5 text-xs text-slate-600">{background}</p></button>)}</div></Section>
      <Section label="Imagem de fundo"><div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-500"><ImageIcon className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-sm font-medium text-slate-700">Imagem personalizada</p><p className="text-xs text-slate-500">PNG, JPG ou WEBP · até 2 MB</p></div><button type="button" className="rounded-md px-2.5 py-1.5 text-xs font-medium text-stone-700 hover:bg-stone-100">Selecionar</button></div></Section>
    </>
  )
}
...
function BehaviorTab({ animation, setAnimation, reveal, setReveal }) {
  ...
  return <><Section label="Animação de entrada"><div className="grid grid-cols-2 gap-2">{ANIMATIONS.map((item) => <button key={item.id} type="button" onClick={() => setAnimation(item.id)} className={['rounded-lg border p-3 text-left transition-colors', animation === item.id ? 'border-stone-400 bg-stone-100' : 'border-slate-200 bg-white hover:bg-slate-50'].join(' ')}><p className="text-sm font-medium text-slate-800">{item.label}</p><p className="mt-0.5 text-xs text-slate-500">{item.description}</p></button>)}</div></Section><Section label="Revelar resultados"><div className="space-y-2">{revealModes.map((item) => <button key={item.id} type="button" onClick={() => setReveal(item.id)} className={['flex w-full items-start gap-3 rounded-lg border p-3 text-left transition-colors', reveal === item.id ? 'border-stone-400 bg-stone-100' : 'border-slate-200 bg-white hover:bg-slate-50'].join(' ')}><span className={['mt-0.5 flex h-4 w-4 items-center justify-center rounded-full border', reveal === item.id ? 'border-stone-600' : 'border-slate-300'].join(' ')}><span className={['h-2 w-2 rounded-full', reveal === item.id ? 'bg-stone-700' : 'bg-transparent'].join(' ')} /></span><div><p className="text-sm font-medium text-slate-800">{item.label}</p><p className="text-xs text-slate-500">{item.description}</p></div></button>)}</div></Section></>
}
...
function Stat({ icon, label, value, color }) {
  const Icon = icon
  const colors = { blue: 'bg-stone-100 text-stone-700', green: 'bg-emerald-50 text-emerald-700' }
